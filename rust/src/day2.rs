...
pub fn day(challenge: u8) {
    let contents = fs::read_to_string("../data/2.txt")
    ...
    if challenge == 1 {
      result = contents.split("\n")
                            .map(|line| {
                              ...
                              let game_number = line[0].split(" ").collect::<Vec<&str>>()
                                .get(1)
                                .expect("Game Number not Present")
                                .parse::<i128>()
                                .expect("Should have been able to parse the game number");
                              let rounds = line[1].split(";")
                                  .all(|round| 
                                    round.split(",")
                                      .all(|turn| {
                                        let turn = turn.split_whitespace().collect::<Vec<&str>>();
                                        let block_count = turn[0].parse::<i128>()
                                          .expect("Should have been able to parse the block count");
                                        if turn[1] == "red" && block_count > 12 || turn[1] == "green" && block_count > 13 || turn[1] == "blue" && block_count > 14 {
                                          return false;
                                        }
                                        return true;
                                      })
